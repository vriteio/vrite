...
Uses Vrite search and GPT-4 with RAG to provide a simple question-answering experience for your Vrite workspace. It streams the answer via [SSEs](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events). Provides the same content filtering options as the `/search` endpoint.
