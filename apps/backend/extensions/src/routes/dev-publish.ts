...
const publishToDEV = async (
  ...
  if (!extension.configuration) throw errors.notFound("extension");
  ...
  if (contentPieceData?.devId) {
    try {
      const response = await fetch(`https://dev.to/api/articles/${contentPieceData.devId}`, {
        method: "PUT",
        headers: {
          "api-key": `${extension.configuration.apiKey}`,
          "Accept": contentType,
...
const devPublishRouter = router({
  ...
  webhook: procedure
    ...
    .input(
      z.object({
        id: zodId(),
        requireCanonicalLink: z.boolean().optional()
      })
    )
    .output(z.void())
    .mutation(async ({ ctx, input }) => {
      ...
      if (!input.requireCanonicalLink || contentPiece.canonicalLink) {
