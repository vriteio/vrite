...
const validateRoleName = async (input: {
  ...
  if (input.excludeRoleID) {
    filters.push(ne(roles.id, toUUID(input.excludeRoleID)));
  }
...
export { duplicateRoleNameError, isRoleNameUniqueViolation, normalizeRoleName, validateRoleName };
