...
import { validateEmail } from "#web/routes/auth/utils";
import { useLocation, useNavigate } from "@solidjs/router";
import { createMutation } from "@tanstack/solid-query";
import { client } from "#web/lib/client";
import { useNotify } from "#web/components/context";
...
const VerifyEmailPage: Component = (props) => {
  ...
  return (
    <div class="flex flex-col">
      <span class="text-2xl font-semibold">Verify email</span>
      <Switch>
        <Match when={view() === "email"}>
          <div class="text-gray-400 dark:text-gray-500 leading-5 text-sm">
            Provide email to verify the code for
          </div>
          ...
          <div class="flex flex-col items-start justify-center w-full transform text-sm text-gray-400 dark:text-gray-500">
            <span>Don't have code to verify?</span>
            <IconButton
              icon="i-lucide:arrow-right"
              variant="text"
              text="primary"
