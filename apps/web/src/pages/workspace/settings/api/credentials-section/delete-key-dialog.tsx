import { Button, createRef, Dialog, IconButton, Input } from "@andesine/components";
...
const DeleteKeyDialog: Component<DeleteKeyDialogProps> = (props) => {
  ...
  return (
    <Dialog opened={props.keys.length > 0} onOverlayClick={handleClose} aria-label="Delete API key">
      <div class="flex flex-col gap-0.5">
        <h3 class="text-lg font-semibold leading-tight">
          Delete {visibleKeys().length > 1 ? `${visibleKeys().length} API keys` : "API key"}?
        </h3>
        <p class="text-sm leading-tight text-gray-400">
          {visibleKeys().length > 1 ? "These keys" : "This key"} will stop working immediately. This
          cannot be undone.
        </p>
      </div>
